    ...
    WKScriptMessageHandler, UIGestureRecognizerDelegate, PullToRefreshDelegate
{
            ...
            if let menuItems = menu["menuItems"] as? [[String: Any]] {
                for menuItem in menuItems {
                    ...
                    let targetMethodName =
                        "onContextMenuActionItemClicked-" + String(self.hash) + "-" + id
                    if !self.responds(to: Selector(targetMethodName)) {
                        let customAction: () -> Void = {
                            ...
                            self.channel?.invokeMethod(
                                "onContextMenuActionItemClicked", arguments: arguments)
                        }
    ...
    public override func canPerformAction(_ action: Selector, withSender sender: Any?) -> Bool {
        if sender as? UIMenuController != nil {
    ...
    override public func observeValue(
        ...
        if keyPath == #keyPath(WKWebView.estimatedProgress) {
            ...
            && change?[NSKeyValueChangeKey.newKey] is String
        {
    ...
    public func takeScreenshot(
        ...
        if let with = with {
            ...
            if #available(iOS 13.0, *),
                let afterScreenUpdates = with["iosAfterScreenUpdates"] as? Bool
            {
    ...
    func setOptions(newOptions: InAppWebViewOptions, newOptionsMap: [String: Any]) {
        ...
        if #available(iOS 11.0, *) {
            ...
            if newOptionsMap["accessibilityIgnoresInvertColors"] != nil
                && options?.accessibilityIgnoresInvertColors
                    != newOptions.accessibilityIgnoresInvertColors
            {
            ...
            if newOptionsMap["contentInsetAdjustmentBehavior"] != nil
                && options?.contentInsetAdjustmentBehavior
                    != newOptions.contentInsetAdjustmentBehavior
            {
                scrollView.contentInsetAdjustmentBehavior =
                    UIScrollView.ContentInsetAdjustmentBehavior.init(
            ...
            && options?.decelerationRate != newOptions.decelerationRate
        {
            scrollView.decelerationRate = Util.getDecelerationRate(
                type: newOptions.decelerationRate)
        }
        ...
        if #available(iOS 9.0, *) {
            ...
            if newOptionsMap["allowsAirPlayForMediaPlayback"] != nil
                && options?.allowsAirPlayForMediaPlayback
                    != newOptions.allowsAirPlayForMediaPlayback
            {
        ...
        if newOptionsMap["allowUniversalAccessFromFileURLs"] != nil
            && options?.allowUniversalAccessFromFileURLs
                != newOptions.allowUniversalAccessFromFileURLs
        {
            configuration.setValue(
                newOptions.allowUniversalAccessFromFileURLs,
                forKey: "allowUniversalAccessFromFileURLs")
    ...
    public func webView(
            ...
            let response = navigationResponse.response as? HTTPURLResponse
        {
            if response.statusCode >= 400 {
                onLoadHttpError(
                    url: response.url?.absoluteString, statusCode: response.statusCode,
                    description: "")
            }
        }
                        ...
                        if result is FlutterError {
                            ...
                            if let r = result {
                                ...
                                case 1:
                                    ...
                                    let permanentPersistence =
                                        response["permanentPersistence"] as? Bool ?? false
                                    let persistence =
                                ...
                                case 2:
                                    ...
                                    {
                                        for (protectionSpace, credentials) in credentialStore
                                            .allCredentials
                                        {
                                            if protectionSpace.host == host
                                                && protectionSpace.realm == realm
                                                && protectionSpace.protocol == prot
                                                && protectionSpace.port == port
                                            {
                                                for credential in credentials {
                                                    InAppWebView.credentialsProposed.append(
                                                        credential.value)
                                                }
                                                break
                                            }
                            ...
                            if let r = result {
                                ...
                                case 1:
                                    ...
                                    let certificatePassword =
                                        response["certificatePassword"] as? String ?? ""

                                    var path: String = certificatePath
                                    do {
                                        path = try Util.getAbsPathAsset(
                                            assetFilePath: certificatePath)
                                    } catch {}

                                    if let PKCS12Data = NSData(contentsOfFile: path),
                                        let identityAndTrust: IdentityAndTrust =
                                            self.extractIdentity(
            ...
            result: { (result) -> Void in
                if result is FlutterError {
                    ...
                    if !handledByClient, InAppWebView.windowWebViews[windowId] != nil {
                        InAppWebView.windowWebViews.removeValue(forKey: windowId)
                        self.loadUrl(
                            urlRequest: navigationAction.request, allowingReadAccessTo: nil)
                    }
                }
            })
    ...
    public func shouldOverrideUrlLoading(
        navigationAction: WKNavigationAction, result: FlutterResult?
    ) {
        channel?.invokeMethod(
    ...
    public func onNavigationResponse(
        navigationResponse: WKNavigationResponse, result: FlutterResult?
    ) {
        channel?.invokeMethod(
    ...
    public func shouldAllowDeprecatedTLS(
        ...
        channel?.invokeMethod(
            "shouldAllowDeprecatedTLS", arguments: challenge.toMap(), result: result)
    }
    ...
    public func userContentController(
        ...
        if message.name.starts(with: "console") {
            ...
            webView.onCallJsHandler(
                handlerName: handlerName, _callHandlerID: _callHandlerID, args: args)
